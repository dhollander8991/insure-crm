...
vi.mock('@/components/charts/revenue-area-chart', () => ({
  RevenueAreaChart: () => <div data-testid="revenue-chart" />,
}));
vi.mock('@/components/charts/policies-donut', () => ({
  PoliciesDonut: () => <div data-testid="policies-donut" />,
}));
vi.mock('@/components/charts/claims-bar-chart', () => ({
  ClaimsBarChart: () => <div data-testid="claims-bar-chart" />,
}));
vi.mock('@/components/charts/pipeline-funnel', () => ({
  PipelineFunnel: () => <div data-testid="pipeline-funnel" />,
}));
vi.mock('@/components/charts/live-coverage-globe', () => ({
...
const mockCustomers = [
  { id: 1, firstName: 'John', lastName: 'Doe', email: 'john@test.com', phone: '050-1111111', israeliId: '123456789', dateOfBirth: '1990-01-01', agentEmail: 'agent@test.com', status: 'ACTIVE' as const },
  { id: 2, firstName: 'Jane', lastName: 'Smith', email: 'jane@test.com', phone: '050-2222222', israeliId: '987654321', dateOfBirth: '1985-05-05', agentEmail: 'agent@test.com', status: 'PROSPECT' as const },
];

const mockPolicies = [
  { id: 1, policyNumber: 'P001', customerId: 1, customerName: 'John Doe', type: 'CAR' as const, status: 'ACTIVE' as const, startDate: '2024-01-01', endDate: '2025-01-01', premium: 200, agentEmail: 'agent@test.com' },
  { id: 2, policyNumber: 'P002', customerId: 2, customerName: 'Jane Smith', type: 'LIFE' as const, status: 'ACTIVE' as const, startDate: '2024-01-01', endDate: '2025-01-01', premium: 300, agentEmail: 'agent@test.com' },
];
...
describe('Dashboard', () => {
  ...
  it('shows correct active policy count', async () => {
    ...
    const activePoliciesCard = screen.getAllByTestId('kpi-card').find(
      card => card.querySelector('[data-testid="kpi-label"]')?.textContent === 'Active Policies'
    );
    expect(activePoliciesCard?.querySelector('[data-testid="kpi-value"]')?.textContent).toBe('2');
  });
  ...
  it('shows new leads count from PROSPECT customers', async () => {
    ...
    const newLeadsCard = screen.getAllByTestId('kpi-card').find(
      card => card.querySelector('[data-testid="kpi-label"]')?.textContent === 'New Leads'
    );
    expect(newLeadsCard?.querySelector('[data-testid="kpi-value"]')?.textContent).toBe('1');
  });
});
