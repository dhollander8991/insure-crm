...
import { policyApi, customerApi } from "@/lib/api";
...
export function PolicyDetailPage() {
  ...
  const { data: customer } = useQuery({
    ...
    queryFn: () => customerApi.getById(policy!.customerId),
    ...
    return (
      <PageTransition>
        <div className="px-4 py-6 md:px-6 lg:px-8">
          <Button variant="ghost" onClick={() => navigate("/policies")} className="mb-4 gap-2">
    ...
    <PageTransition>
      <div className="relative px-4 py-6 md:px-6 lg:px-8">
        ...
        <div className="relative">
          <Button variant="ghost" onClick={() => navigate("/policies")} className="mb-6 gap-2">
            <ArrowLeft className="h-4 w-4" /> Back to Policies
          </Button>
          ...
          >
              ...
              <CardHeader>
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <CardTitle className="mt-1 text-xl">{policy.policyNumber}</CardTitle>
                  </div>
                  <div className={cn("flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br text-lg font-bold", typeColors[policy.type])}>
                    {policy.type[0]}
                  </div>
                </div>
                <Badge variant="outline" className={cn("w-fit font-medium", statusStyles[policy.status])}>
                  {policy.status}
                </Badge>
              </CardHeader>
              <CardContent>
                <Row label="Type" value={policy.type} />
                <Row label="Premium" value={`$${policy.premium}/mo`} />
                <Row label="Start Date" value={new Date(policy.startDate).toLocaleDateString()} />
                <Row label="End Date" value={new Date(policy.endDate).toLocaleDateString()} />
                <Row label="Agent" value={policy.agentEmail} />
              </CardContent>
            </Card>
            ...
            <Card className="bg-card/70 backdrop-blur-xl">
              ...
              <CardContent>
                {customer ? (
                  <Link to={`/clients/${customer.id}`} className="block">
                ...
                <div className="mt-6 space-y-3 border-t pt-4">
                  <div className="flex items-center gap-3 text-sm text-muted-foreground">
                    ...
                    <span>Expires {new Date(policy.endDate).toLocaleDateString(undefined, { year: "numeric", month: "long", day: "numeric" })}</span>
                  </div>
                  <div className="flex items-center gap-3 text-sm text-muted-foreground">
